...
export default async function handler(req, res) {
  ...
  try {
    ...
    const enrichedZiCompanyId =
      r.companyId
        ? String(r.companyId)
        : r.employmentHistory?.[0]?.company?.companyId
    ...
    if (!resolvedCompanyId && enrichedZiCompanyId) {
      const { data: byZi } = await supabase
        ...
        .eq('zi_company_id', enrichedZiCompanyId)
        .limit(1).maybeSingle()
      if (byZi) resolvedCompanyId = byZi.id
    }
    if (!resolvedCompanyId && enrichedCompanyName) {
      const { data: byName } = await supabase
        .from('companies').select('id')
        .ilike('name', enrichedCompanyName.trim())
        .limit(1).maybeSingle()
      if (byName) resolvedCompanyId = byName.id
    }
    ...
    const { data: updated, error: updateErr } = await supabase
      ...
      .update({
        ...
        company_name:  enrichedCompanyName,
        zi_company_id: enrichedZiCompanyId,
